...
from engines.full_finetuning import train_one_epoch
...
def main(args):
    ...
    optimizer_discriminator = torch.optim.AdamW(model_without_ddp.gan_losses.parameters(), lr=args.lr, betas=(0.9, 0.95))
    loss_scaler = NativeScaler()
    loss_scaler_discriminator = NativeScaler()

    misc.load_model(args=args, model_without_ddp=model_without_ddp, ema=ema)
    misc.resume_model(args=args, model_without_ddp=model_without_ddp, ema=ema, optimizer=optimizer, optimizer_discriminator=optimizer_discriminator, loss_scaler=loss_scaler, loss_scaler_discriminator=loss_scaler_discriminator)

    if global_rank==0:
        wandb.init(project="karl", group="imagenet100", name="karl_stage_2", config=args)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_one_epoch(
            ...
            optimizer, optimizer_discriminator, device, epoch, loss_scaler, loss_scaler_discriminator,
        ...
        if args.output_dir and (epoch % 20 == 0 or epoch + 1 == args.epochs):
            misc.save_model(
                ...
                loss_scaler=loss_scaler, loss_scaler_discriminator=loss_scaler_discriminator, epoch=epoch)
